...
pub fn parse_midi(path: &str) -> Result<(MidiSummary, MidiData), String> {
    ...
    // Convert ticks to milliseconds with tempo changes
    ...
    // Collect MIDI channel events from all tracks with absolute timing
    let mut all_events: Vec<(u64, usize, String)> = Vec::new(); // (time_ms, track_index, event_kind)

    for (track_idx, track) in smf.tracks.iter().enumerate() {
        let mut cumulative_ticks: u64 = 0;
        for event in track {
            cumulative_ticks += event.delta.as_int() as u64;
            // Only include MIDI channel messages, filter out Meta events
            if let TrackEventKind::Midi { message, .. } = event.kind {
                let time_ms = ticks_to_ms(cumulative_ticks);
                let event_kind = format!("{:?}", message);
                all_events.push((time_ms, track_idx, event_kind));
            }
        }
    }

    // Sort chronologically by time
    all_events.sort_by_key(|(time_ms, _, _)| *time_ms);

    // Format events
    let events: Vec<String> = all_events
        .iter()
        .map(|(time_ms, track_idx, event_kind)| {
            format!("[T{}] {}ms : {}", track_idx, time_ms, event_kind)
        })
        .collect();

    // Find last NoteOff event in the last track
    let last_note_time: u64 = if let Some(last_track) = smf.tracks.last() {
        let mut cumulative_ticks: u64 = 0;
        let mut last_note_off_ticks: u64 = 0;

        for event in last_track {
            cumulative_ticks += event.delta.as_int() as u64;
            if let TrackEventKind::Midi { message, .. } = event.kind {
                match message {
                    midly::MidiMessage::NoteOff { .. } => {
                        last_note_off_ticks = cumulative_ticks;
                    }
                    midly::MidiMessage::NoteOn { vel, .. } if vel == 0 => {
                        // NoteOn with velocity 0 is effectively NoteOff
                        last_note_off_ticks = cumulative_ticks;
                    }
                    _ => {}
                }
        ...
        ticks_to_ms(last_note_off_ticks)
    } else {
        0
    };

    // Build MidiData using default mode (Python21) and no transpose for caching
    let midi_data = load_midi(path, NoteMode::Python21, 0)?;
